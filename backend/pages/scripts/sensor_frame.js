...
class CSCMetrics {
    constructor() {
        ...
        this.speedMs    = 0;
        this.cadenceRpm = 0;
        // Track how many consecutive notifications had no new revolution
        this._frozenCount = 0;
    ...
    update(dataView) {
        ...
        if (revKey !== null && revKey === this._lastRevKey) {
            // Same cumulative count — sensor is stopped
            this._frozenCount++;
...
function setBleState(state, text) {
    ...
    // Hide connect button once connected, show it when not
...
function onCSC(event) {
    metrics.update(event.target.value);
